...
export default function HomeClient({ products }: HomeClientProps) {
  ...
  // Enable hero animation after client hydration
  useEffect(() => {
    setMounted(true);
  }, []);
  ...
  const featuredProducts = safeProducts.filter((product: any) => product.isFeatured).slice(0, 6);

  // Get seasonal sale products (products that are on sale)
  const seasonalProducts = safeProducts.filter((product: any) => product.isOnSale).slice(0, 6);

  const currentTabProducts = safeProducts.filter((product: any) => {
